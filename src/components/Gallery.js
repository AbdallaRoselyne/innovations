...
import { Search, Play, Pause, Volume2, VolumeX, Maximize, X } from "lucide-react";
...
const CLOUDINARY_URL = `https://res.cloudinary.com/${cloudName}`;
  ...
  const galleryItems = [
    ...
    },
    {
      ...
      category: "Electric mobility",
      src: "Electric_Scooter_-01-Original.jpg",
      alt: "EV Prototype",
    ...
    <section className="py-20 px-4 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 md:mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent mb-4 md:mb-6">
            Innovation Gallery
          </h2>
          <div className="w-20 md:w-32 h-1 bg-gradient-to-r from-blue-500 to-purple-500 mx-auto rounded-full"></div>
          <p className="mt-4 md:mt-6 text-base md:text-xl text-gray-600 max-w-2xl mx-auto px-4">
            Explore our cutting-edge projects in digital construction, electric mobility, and clean farming
          </p>
        </div>
          ...
          <div className="flex justify-center px-2">
            <div className="w-full max-w-4xl overflow-x-auto">
              <div className="flex sm:inline-flex rounded-2xl p-1 bg-white/60 backdrop-blur-sm border border-white/20 shadow-xl min-w-max mx-auto">
                {["all", "Digital construction", "Electric mobility", "Clean farming"].map((tab) => (
                  <button
                    key={tab}
                    ...
                    className={`px-3 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-medium rounded-xl transition-all duration-300 whitespace-nowrap ${
                      activeTab === tab
                        ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg transform translate-y-[-1px] sm:translate-y-[-2px]"
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-12 md:py-16 px-4">
            ...
            <h3 className="text-lg md:text-xl font-semibold text-gray-600 mb-2">No results found</h3>
          ...
          <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-2 md:p-4">
            ...
            <div className="max-w-5xl max-h-[90vh] w-full">
              ...
              <div className="mt-2 md:mt-4 text-center text-white px-4">
                <h3 className="text-lg md:text-2xl font-bold mb-1 md:mb-2">{selectedMedia.name}</h3>
                <p className="text-sm md:text-base text-gray-300">{selectedMedia.category}</p>
              </div>
            </div>
          </div>
        )}
